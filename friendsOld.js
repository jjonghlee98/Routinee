...
let sr_friends = document.querySelectorAll(".sr_friend"); // 친구검색 친구
let sr_friendsname = document.querySelectorAll('.sr_friend .friend_name');
let sr_friendsimg = document.querySelectorAll('.sr_friend img');
let follow_btn = document.querySelectorAll(".bxs-user-plus"); // 친구요청 버튼
let s_selected = 0; // 받은요청이 선택되어있는지 여부
...
for(let i = 0; i < follow_btn.length; i++) {
  follow_btn[i].addEventListener('click', () => {
    ...
    let new_sfriend = document.createElement('div');
    new_sfriend.classList.add('friend');
    new_sfriend.classList.add('s_friend');
    if(!s_selected) {
      new_sfriend.classList.add('hidden');
    }

    let new_sfriend_img = document.createElement('img');
    new_sfriend_img.src = sr_friendsimg[i].src;

    let new_name = document.createTextNode(sr_friendsname[i].innerText);
    let new_sfriend_name = document.createElement('span');
    new_sfriend_name.appendChild(new_name);
    new_sfriend_name.classList.add('friend_name');

    let new_cancelbtn = document.createElement('button');
    new_cancelbtn.appendChild(document.createTextNode("요청취소"));
    new_cancelbtn.classList.add('cancel_button');

    new_sfriend.appendChild(new_sfriend_img);
    new_sfriend.appendChild(new_sfriend_name);
    new_sfriend.appendChild(new_cancelbtn);

    request_list.appendChild(new_sfriend);
    console.log(1);

    sr_friends[i].remove();

    new_cancelbtn.addEventListener('click', () => {
      alert(sr_friendsname[i].innerText + '님에게 보낸 친구요청을 취소했습니다');
      new_sfriend.remove();
    })
  })
}